...
function renderMainShell() {
  ...
  if (state.activeTab === 'inicio') {
    ...
    if (state.currentView === 'measures') renderMeasures(main);
...
function renderExerciseListForSelector(query = '') {
  ...
  return Object.keys(EXERCISE_DB)
    .filter(id => EXERCISE_DB[id].name.toLowerCase().includes(q) || (MUSCLE_GROUPS[EXERCISE_DB[id].muscleGroup] && MUSCLE_GROUPS[EXERCISE_DB[id].muscleGroup].name.toLowerCase().includes(q)))
    .map(id => {
      const ex = EXERCISE_DB[id];
      const m = MUSCLE_GROUPS[ex.muscleGroup] || {color:'#000', name:'Otro'};
      return `
        <div class="home-action-card" onclick="addExerciseToEmptySession('${id}')" style="margin-bottom:0px; cursor:pointer; padding:15px; border:2px solid #000; background:#fff; border-radius:0; display:flex; gap:15px; align-items:center;">
...
function toggleSet(exerciseIndex, setIndex, btnElement = null) {
  ...
  let setsDoneInDay = 0;
  day.exercises.forEach((ex, i) => {
    const k = `day${state.currentDay}_ex${i}`;
...
function addSet(exerciseIndex) {
  const exerciseConfig = state.routine.days[state.currentDay].exercises[exerciseIndex];
  if(exerciseConfig.sets >= 10) return showToast('⚠️', "Límite: 10 series");
  
  exerciseConfig.sets++;
  const setKey = `day${state.currentDay}_ex${exerciseIndex}`;
...
function removeExercise(exerciseIndex) {
  ...
  // Re-map the completedSets keys to shift them down.
  // This is tricky, a simpler way is to clear the sets of the removed ex, and shift everything down.
  ...
  return `
    <div class="workout-summary ${allDone ? 'completed' : ''}" id="workoutSummary">
      ...
      <div class="summary-stats">
        ...
        <div class="summary-stat">
          <div class="summary-stat-value">${totalSets > 0 ? Math.round((completedCount / totalSets) * 100) : 0}%</div>
          <div class="summary-stat-label">Progreso</div>
        </div>
        <div class="summary-stat">
...
function resetWorkout(skipConfirm = false) {
  ...
  // Solo reseteamos la sesión actual
  const day = state.routine.days[state.currentDay];
  day.exercises.forEach((ex, i) => {
  ...
  stopRestTimer();
  saveState();
  renderApp(); // This will re-render everything correctly
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
